// src/app/not-found.tsx
import Link from 'next/link'

function CuteGhost() {
  return (
    <svg 
      viewBox="0 0 100 100" 
      className="w-32 h-32 mb-6 animate-bounce-slow text-[#2fcac1]"
    >
      {/* Ghost body */}
      <path
        d="M15 50 Q15 15 50 15 Q85 15 85 50 L85 85 Q85 90 80 85 L65 70 Q60 65 55 70 L50 75 L45 70 Q40 65 35 70 L20 85 Q15 90 15 85 Z"
        fill="currentColor"
        className="opacity-80"
      />
      {/* Eyes */}
      <circle cx="35" cy="45" r="5" className="fill-background"/>
      <circle cx="65" cy="45" r="5" className="fill-background"/>
      {/* Cute smile */}
      <path
        d="M40 60 Q50 70 60 60"
        fill="none"
        stroke="currentColor"
        strokeWidth="2"
        className="stroke-background"
      />
    </svg>
  )
}

export default function NotFound() {
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] text-center px-4">
      <CuteGhost />
      <h1 className="text-6xl font-bold mb-4">404</h1>
      <p className="text-base mb-8 text-foreground/70">
        oops! looks like you've wandered into uncharted territory
      </p>
      <Link 
        href="/" 
        className="px-6 py-3 rounded-full bg-foreground/10 text-foreground hover:bg-foreground/20 transition-colors"
      >
        Head Back Home
      </Link>
    </div>
  )
}
