  ...
  return (
    <nav className="w-full px-2 sm:px-0">
      <div className="w-full overflow-x-auto">
        <div className="border sm:border-foreground/20 rounded-full px-1 py-1 flex items-center justify-between w-full relative sm:border">
          {/* Logo (Left) */}
          ...
          <button 
            onClick={toggleMenu} 
            className="sm:hidden text-xl focus:outline-none ml-2"  // Added ml-2 for a little left margin
              ...
              <Link
                ...
                className={`text-lg sm:text-lg px-4 sm:px-3 py-1.5 rounded-full transition-colors ${
                  pathname === item.path 
                    ...
                    : 'hover:bg-foreground/5 hover:text-blue-500'
                }`}
              >
                {item.name}
              </Link>
        ...
        {/* Mobile Menu (Overlay) */}
        <div
          className={`sm:hidden fixed inset-0 bg-black/50 z-50 ${isMenuOpen ? 'block' : 'hidden'}`}
          onClick={() => setIsMenuOpen(false)} // Close the menu if the overlay is clicked
        >
          <div className={`absolute top-0 left-0 w-full bg-white p-8 flex flex-col items-center space-y-4 ${isMenuOpen ? 'block' : 'hidden'}`}>
            {navItems.map((item) => (
              <Link
                key={item.name}
                href={item.path}
